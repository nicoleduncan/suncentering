FUNCTION makeskinnylimbstrips, inputstruct, inputimage
;+
;   :Description:
    ...
    FOR i = 0, !param.nstrips - 1 DO BEGIN
        ...
        IF row_where[0] NE -1 THEN BEGIN
            ...
            ; If chord is too long, it tries to crop from outside of image file
                (a[jj].xstrips[i].array)[row_where[0]-1:row_where[0]]   
            ...
            a[jj].limbxstrips[i].begindex   = FIX(row_where[0])
        ENDIF
        IF row_where[-1] NE -1 THEN BEGIN
            a[jj].limbxstrips[i].endpoints  = (a[jj].xstrips[i].array)[row_where[-1] - 1:row_where[-1]]   
            a[jj].limbxstrips[i].endindex   = FIX(row_where[-1])
        ENDIF

        IF col_where[0] NE -1 THEN BEGIN
            a[jj].limbystrips[i].startpoints  = (a[jj].ystrips[i].array)[col_where[0]- 1:col_where[0]]   
            a[jj].limbystrips[i].begindex     = FIX(col_where[0])
        ENDIF
        IF col_where[-1] NE -1 THEN BEGIN
            a[jj].limbystrips[i].endpoints  = (a[jj].ystrips[i].array)[col_where[-1]- 1:col_where[-1]]   
            a[jj].limbystrips[i].endindex     = FIX(col_where[-1])
        ENDIF
    ENDFOR
endfor
