...
PRO centerv3,file=file,ministrip_length=ministrip_length,scan_width=scan_width,time=time,order=order, $
...
;
;       Different from v2 in that this one is slightly more organized and uses a different version of
;       comp2.
;       
;       Important!:
...
thresh = max(cropped_image)-2*stddev(cropped_image)
; **************
...
;So the new way is 2x better... not much?

sum     = TOTAL(sunonly)
xpos    = TOTAL( TOTAL(sunonly, 2) * Indgen(n_col) ) / sum
ypos    = TOTAL( TOTAL(sunonly, 1) * Indgen(n_row) ) / sum 
f = systime(1,/s)
print,'Old way took: ',f-e,' seconds'
print,xpos,ypos

j=systime(1,/s)

FOR i = 0,n_col-1 DO BEGIN
    a=where(cropped_image[i,*] gt thresh)
    ysum += total( (cropped_image[i,*])[a] * a )
ENDFOR
FOR i = 0,n_row-1 DO BEGIN
    b=where(cropped_image[*,i] gt thresh)
    xsum += total( (cropped_image[*,i])[b] * b )
ENDFOR
xsum/=total(cropped_image[where(cropped_image gt thresh)])
ysum/=total(cropped_image[where(cropped_image gt thresh)])
k=systime(1,/s)
print,''
print,'New way took: ',k-j,' seconds'
print,xsum,ysum
print,''
print,'Newer way is ', (f-e)/(k-j),' times faster... not much imo. HOWEVER:'
print,"This method is robust in that it doens't care if there are holes in the array"

ysum=0
xsum=0
l = systime(1,/s)
FOR i = 0,n_row*n_col-1 DO BEGIN
    xind = i mod n_col
    yind = i/n_col
    a=where(cropped_image[xind,*] gt thresh)
    if n_elements(a) ge 1 and where(b eq -1) ne 1 then ysum += total( (cropped_image[xind,*])[a] * a )
    b=where(cropped_image[*,yind] gt thresh)
    if where(b eq -1) ne 1 then xsum += total( (cropped_image[*,yind])[b] * b )
ENDFOR
xsum/=total(cropped_image[where(cropped_image gt thresh)])
ysum/=total(cropped_image[where(cropped_image gt thresh)])
y = systime(1,/s)
print,''
print,'Newer way with mod took: ',y-l," seconds. Now, why is this so much slower? And it's not even right. Look:"
print,xsum,ysum
...
;THIS IS BEAUTIFUL
sunonly = suncheck*cropped_image
;God damit this isn't fair how fast this is.
sum     = TOTAL(sunonly)
xpos    = TOTAL( TOTAL(sunonly, 2) * Indgen(n_col) ) / sum
ypos    = TOTAL( TOTAL(sunonly, 1) * Indgen(n_row) ) / sum 
f = systime(1,/s)
print,'An even newer way took: ',f-e,' seconds'
print,xpos,ypos
stop
