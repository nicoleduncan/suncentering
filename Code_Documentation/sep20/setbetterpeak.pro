...
if !param.peakfind_buff eq 13000 then arr = arr[0:N_ELEMENTS(arr)-10000]
...
if nsuns gt 1 then begin
    for i = 0,nsuns-1 do begin
        ...
        arr=arr[0: N_ELEMENTS(arr) - !param.peakfind_buff]
    endfor
endif else begin
    peakarr = MEAN(WHERE(a[0:N_ELEMENTS(arr) - !param.peakfind_buff] eq MAX(a[0:N_ELEMENTS(arr) - !param.peakfind_buff])))
endelse
