...
Once we had a list of x and y positions of possible fiducial coordinates, we looked at each possibility and checked to see if it passed certain parameters. First, we checked to see if it was dark enough. If the fiducial coordinate was on the solar disk but not on a fiducial, it would be much brighter than what a fiducial could be, so it's ruled out. Second, we check to see if it's within a certain distance form the center of the sun. This is so that dark fiducials off the edge of the sun aren't counted. Once we narrowed down the possible locations for fiducials, we went and cropped a small box around the fiducial and looked at the 1D sums. If the fiducial candidate is indeed a fiducial, there will be a characteristic dip in brightness in both directions, as seen in \cref{fidcand_sums}. \Cref{fidcand_sums_falsepos} shows the 1D sum profiles of a fiducial candidate which is on a solar limb. 
...
\begin{figure}[!ht]
    ...
    \begin{subfloatrow}[2]%
       ...
       {%
       \caption{The profiles of a real fiducial}%
       \label{fidcand_sums}%
       }%
        \ffigbox[\Xhsize]%
       {%
...
The other method we use is to process the chord ignorantly, ignoring fiducial positions. When we end up calculating them, we go back and make sure the chords do not contain fiducials near the solar limbs. If they do, we create a new set of chords, fiducial free, and reprocess the limbs/limb fitting.
